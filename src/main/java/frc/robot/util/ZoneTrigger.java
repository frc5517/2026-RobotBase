...
public class ZoneTrigger {
    ...
    private List<Pair<Translation2d, Translation2d>> ifShouldFlip(List<Pair<Translation2d, Translation2d>> zones) {
        ...
        if (AllianceFlipUtil.shouldFlip()) {
            ...
            zones.forEach(pair -> {
                // Load our updated list with the first and second flipped to properly flip the width and heights as well.
    ...
    /**
     * Updates the {@link Rectangle2D}s relative to the current Alliance.
     * Alliance doesn't like being checked at runtime, so it needs to be checked slightly later.
     */
    ...
    /**
     * Sets both the Zone and Trigger publishers with the same network table.
     * The trigger will be in a "Triggers" subTable.
     *
    ...
    /**
     * Sets both the Zone and Trigger publishers with the same network table path.
     * The trigger will be in a "Triggers" subTable.
     * For Example, Smartdashboard/RobotTelemetry/ZonePublishers
     *
