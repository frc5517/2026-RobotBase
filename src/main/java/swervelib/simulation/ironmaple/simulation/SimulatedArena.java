...
public abstract class SimulatedArena {
    ...
    public void addValueToMatchBreakdown(boolean isBlueTeam, String ValueKey, double toAdd) {
        if (isBlueTeam) {
            blueScoringBreakdown.merge(ValueKey, toAdd, Double::sum);
        } else {
            redScoringBreakdown.merge(ValueKey, toAdd, Double::sum);
        }
    }
    ...
    public synchronized List<GamePiece> getGamePiecesByType(String type) {
        final List<GamePiece> gamePieces = new ArrayList<>(this.gamePieces);
      return gamePieces.stream().filter(gamePiece -> Objects.equals(gamePiece.getType(), type)).toList();
    }
